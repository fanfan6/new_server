...
def search_info(request):
    ...
    #        'service': 'S06'
        ...
        user_info['report_id'] = '100273151599925189875'
        ...
        try:
            ...
            if res_data['is_black'] == 1:
                response['is_black'] = u'是'
            elif res_data['is_black'] == 0:
                response['is_black'] = u'否'
            response['model_score'] = res_data['model_score']
            response['credit_score'] = res_data['credit_score']
            response['sub_features'] = res_data['sub_features']
        #    response['report_id'] = res_data['report_id']
            response['report_id'] = '100273151599925189875'
            user_info['report_id'] = response['report_id']
            user_info['create_time'] = int(time.time())
...
def service_info(request):
    try:
        ...
        # res = models.ReportRecord.objects.filter(**user_info).order_by('-create_time')[:50]
        res = models.ReportRecord.objects.filter(**user_info).order_by('-create_time')[:1]
        print res
    except:
        res = {}

    return render(request, 'service.html', {'res': res})
